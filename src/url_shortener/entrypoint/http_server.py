...
from url_shortener.presentation.web_api.get_full_url_by_short_url import get_full_url_by_short_url
...
class HTTPRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        try:
            ...
            response_presenation = asyncio.run(create_short_url(ioc=self.ioc, request=CreateShortUrlRequest(data['key'])))

            post_data_answer = {'id': str(response_presenation.id),
                                'full_url': response_presenation.full_url,
                                'short_url': response_presenation.short_url
                                }

            response = {'message': 'This is a POST request', 'data_received': post_data_answer}
    ...
    def do_GET(self):
        try:
            ...
            response_presenation = asyncio.run(get_full_url_by_short_url(ioc=self.ioc, request=GetFullUrlByShortUrlRequest(query_params['full_url'][0])))
...
def create_app(domain_url: DomainUrl,
               database: dict,
               id_generator_short_url,
               port: int = 8080):
    ioc = IoC(domain_url=domain_url, database=database,
              id_generator_short_url=id_generator_short_url)

    http_server = get_http_server(server_class=HTTPServerWithIoc, handler_class=HTTPRequestHandler, port=port, ioc=ioc)
